...
class Hamiltonian_System(object):
    ...
    def numerical_solve(self, xs, dt, nstep, phi_init=None):
...
class comb_potential(object):
    def __init__(self, qw, iw, pweight, pa=1.):
        ...
        self.qw = qw
        self.iw = iw
        self.qp = quad_potential(pweight).potential_eval
...
class quadratic_init(object):
    def __init__(self, phiweight):
        self.phiweight = phiweight
        
    def func(self, x):
        y = torch.matmul(x, self.phiweight)
        quad = - torch.sum(y**2/2., dim=1, keepdim=True) # + torch.matmul(y, self.c)
        return quad
    
    
    
def dp_evaluate(H_system, flow, flow_auxil, pstate, eta, samples, device='cpu'):
    xs, log_jac = flow(samples)
    copy_net(flow, flow_auxil)
    xs_auxil = flow_auxil(samples)
