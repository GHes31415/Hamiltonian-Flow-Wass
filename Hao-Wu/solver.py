...
def dtheta_eval(energy, flow, flow_auxil, G_samples, E_samples, lstr=2e-3):
    xs, logj = flow(E_samples)
    #KL_loss = relative_entropy_loss(zk, log_jacobians)
    loss = energy(xs, logj)
    grad_H = torch.autograd.grad(loss, flow.parameters(), allow_unused=True, retain_graph=True, create_graph=True)
    vec_grad_H = nn.utils.parameters_to_vector(grad_H)

    def mv_operator(vec):
        mv_prod = G_op(flow, flow_auxil, G_samples, vec)
        return tensor_to_numpy(mv_prod)

    nparams = len(vec_grad_H)
    Gop = LinearOperator((nparams, nparams), matvec=mv_operator)

    dH = tensor_to_numpy(vec_grad_H)
    dtheta, info = scipy.sparse.linalg.minres(Gop, dH, tol=lstr, maxiter=None, M=None, callback=None)
    err = Gop.matvec(dtheta) - dH
    errnorm = np.linalg.norm(err, 2)
    errrel = errnorm/np.linalg.norm(dH, 2)
    return dtheta, err, errrel
...
def explicit_Euler_solver(H_system, flow, flow_auxil, pstate, h, samples, lstr, px_iters, lr, solver_type='explicit', eta_guess=None, print_type=None, pe_samplesize=None):
...
def fixed_point_optim_solver(H_system, flow, flow_auxil, pstate, h, samples, sample_log_rho, z_log_rho_func,
                             lstr, px_iters, lr, solver_type='explicit', eta_guess=None, print_type=None, pe_samplesize=None):
    alpha_k = nn.utils.parameters_to_vector(flow.parameters())
    device = alpha_k.device
    alpha_k = tensor_to_numpy(alpha_k)
    ...
    if print_type:
        print('initial residual')
        print("first", residual_evaluate(flow, flow_auxil, eta, pstate, samples))
    for i in range(px_iters):
        ...
        if print_type:
            ...
            print(residual_evaluate(flow, flow_auxil, eta, pstate, samples))
            print('grad of eta loss:', etagrad)
    #print("going to update parameters")
    alpha = torch.Tensor(alpha_k).to(device) + h * eta
    nn.utils.vector_to_parameters(alpha, flow.parameters())
    copy_net(flow, flow_auxil)
...
#     else:
#         pe_samples = sample_z(pe_samplesize, samples.shape[1], device)
    dp = dp_evaluate(H_system, flow, flow_auxil, pstate, eta, samples, sample_log_rho, z_log_rho_func)
    #print("Evaluate dp_dt:", dp)
    p = pstate + h * dp
    return tensor_to_numpy(eta), p, res, err
